...
import { WHITE_BULLETS } from "./QueenConfig";

const DIRECTIONS = 20;

const PATTERN: [number, number, number][] = Array(DIRECTIONS).fill([1, 0, 1]);
...
export class WhiteAttack extends BaseAttack {
    private whiteBulletsNode: TransformNode;
    ...
    constructor(
        ...
        this.whiteBulletsNode = new TransformNode("white-bullets-node");
        this.whiteBulletsNode.parent = parent;
        this.whiteBulletsNode.metadata = { config: DUMMY_ENEMY_CONFIG };

        this.shooter = new EnemyShooter(this.scene, this.whiteBulletsNode);
        this.shooter.shooterEnabled = false;

        this.delays = WHITE_BULLETS;
        this.count = count;
    ...
    public override dispose() {
        ...
        if (!this.whiteBulletsNode.isDisposed()) {
            this.whiteBulletsNode.dispose();
        }
