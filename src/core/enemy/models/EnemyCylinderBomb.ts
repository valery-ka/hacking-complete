...
export class EnemyCylinderBomb extends Enemy {
    ...
    public createMeshInstance() {
        ...
        this.mesh = assets.enemy_cylinder_bomb_merged.createInstance(
            `enemy-minion-cylinder-bomb-${this.index}`,
        );
        ...
        if (this.config.follow_player.enabled) {
            ...
            this.collider = MeshBuilder.CreateCylinder(
                `enemy-collider-cylinder-bomb-${this.index}`,
