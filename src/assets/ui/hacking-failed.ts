...
export const createHackingFailedUI = (scene: Scene, layer: UtilityLayerRenderer) => {
    const uiMain = AdvancedDynamicTexture.CreateFullscreenUI("UI-Hacking-Failed-Main", true, scene);
    const uiText = AdvancedDynamicTexture.CreateFullscreenUI(
        "UI-Hacking-Failed-Text",
        true,
...
const createTextBlock = (ui: AdvancedDynamicTexture) => {
    const text = new TextBlock("hacking-failed", TEXT);
...
export const animateOpacity = (ui: AdvancedDynamicTexture, duration: number = 125) => {
    ...
    const animate = () => {
        if (!isTextureAlive(ui) || !ui.metadata) return;
...
export const animateRootOpacity = (container: Container, duration: number = 100) => {
    ...
    const animate = () => {
        if (!isControlAlive(container)) return;
